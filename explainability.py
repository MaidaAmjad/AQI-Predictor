...
def _sample_rows(X: pd.DataFrame, max_rows: int) -> pd.DataFrame:
    if len(X) <= max_rows:
        return X
    return X.sample(max_rows, random_state=42)


def _shap_explainer(model, X_background: pd.DataFrame):
    import shap
...
def compute_shap_summary(
    ...
    *,
    ...
    X_bg = _sample_rows(X_train, max_background)
    X_ex = _sample_rows(X_test, max_explain)
    explainer, explainer_type = _shap_explainer(model, X_bg)
...
def build_explainability_report(
    ...
    X_train: pd.DataFrame,
    X_test: pd.DataFrame,
    x_instance: pd.Series | None = None,
) -> dict:
